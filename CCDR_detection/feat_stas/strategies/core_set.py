...
class CoreSet(Strategy):
    ...
    def furthest_first(self, X, X_set, n, metric):
        m = np.shape(X)[0]
        ...
        for i in tqdm(range(n)):
            idx = min_dist.argmax()
            idxs.append(idx)
            dist_new_ctr = pairwise_distances(X, X[[idx], :], metric)
            for j in range(m):
                min_dist[j] = min(min_dist[j], dist_new_ctr[j, 0])

        return idxs

    def query(self, n, embedding, idxs_lb, metric):
        ...
        chosen = self.furthest_first(embedding[idxs_unlabeled, :], embedding[lb_flag, :], n, metric)
