...
def _get_img_generator_fn(img_path_list, reshape_to, max_count=-1):
...
def compute_embeddings_for_dir(
  ...
  Args:
    ...
    max_count: Max number of images in the directory to use. A
  ...
  print(f'Calculating embeddings for {count} images.')
  ...
  dataset = dataset.batch(per_device_batch_size, drop_remainder=False)
